...
----------------------------------------------------
		<h3>이름 : ${pet.name }</h3>
		${pet }
		<c:set var="pet_no" value="${pet.pet_no-1 }"></c:set>
		${pet_no }
<%-- 		<c:if test="${pet_image_no>0 }"> --%>
			<img src="${pageContext.request.contextPath }/member/pet/image?pet_no=${pet_no}" style="max-width: 40%; height: auto;">
<%-- 		</c:if> --%>
		<h3>나이 : ${pet.age }</h3>
		<h3>동물종 : ${pet.type }</h3>
		<h3>${pet.ect }</h3>
